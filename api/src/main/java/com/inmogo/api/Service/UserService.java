...
public class UserService {
     ...
     */
    public UserTemplate findUserByEmail(String email){
        return userRepo.findUserByEmail(email).orElseThrow(() -> new RuntimeException("Usuario no encontrado."));
    }
     ...
     * @return El usuario si existe. un RuntimeException si no.
     */
    public UserTemplate findUserById(Long id){
        return userRepo.findUserById(id).orElseThrow(() -> new RuntimeException("Usuario no encontrado."));
    }
    ...
    public UserTemplate register(UserTemplate tempUser){
        if (userRepo.findUserByEmail(tempUser.getEmail()).isPresent()){
            throw new RuntimeException("Ya hay una cuenta con ese correo.");
        }
        if (userRepo.findUserByRut(tempUser.getRut()).isPresent()){
    ...
    public UserTemplate login(String email, String password){
        ...
        UserTemplate temp = userRepo.findUserByEmail(email).orElseThrow(() -> new RuntimeException("Credenciales Invalidas."));
